...
def generate_purity_csv(
    ...
    for img_idx, (xs, _) in project_iter:
        ...
        for proto in range(num_prototypes):
            ...
            if torch.max(classification_weights[:, proto]).item() > 1e-5:
                scores_per_prototype[proto].append((img_idx, pooled_scores[proto].item()))
    ...
    proto_iter = tqdm(active_protos, total=len(active_protos), desc='Pass 2/2: Writing CSV')
    for proto in proto_iter:
        ...
        for _, row in topk.iterrows():
            ...
            max_val_h, max_idx_h = torch.max(proto_map, dim=0) # Max across H for each W
            ...
            proto_img_coordinates.append([proto, img_path_str, h_min, h_max, w_min, w_max])

    if low_similarity_protos:
        print(f"Warning: {len(low_similarity_protos)} prototypes have top-k similarity < 0.1.")
...
def eval_purity_from_csv(csv_path: Path, cfg: PurityBenchConfig, latent_wshape: int = None):
    """
        ...
        for row in reader:
            ...
            if img_id in img_to_part_xy_vis:
                ...
                for part_id, (x, y) in part_dict_img.items():
                    part_in_patch = 0
                    if (orig_h_min <= y <= orig_h_max) and (orig_w_min <= x <= orig_w_max):
                        part_in_patch = 1
    ...
    if max_purity_values:
        ...
        print(f"Mean purity of prototypes (purest part): {mean_pur:.4f} (std: {std_pur:.4f})")
    else:
