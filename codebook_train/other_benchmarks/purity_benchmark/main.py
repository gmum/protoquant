...
def run_pipnet(args=None):
    ...
    logger.info("Device used: %s with id %s", device, device_ids)
    ...
    if len(classes) <= 20:
        if args.validation_size == 0.0:
            logger.info("Classes: %s", testloader.dataset.class_to_idx)
        else:
    ...
    # If using ProtoQuantNet, wrap it with ProtoQuantAdapter for compatibility with PIPNet-based benchmark code
    try:
        from src.models.proto_quantnet import ProtoQuantNet
        from pipnet.protoquant_adapter import ProtoQuantAdapter

        if isinstance(net, ProtoQuantNet):
            net = ProtoQuantAdapter(net)
            logger.info(
                "Wrapped ProtoQuantNet with ProtoQuantAdapter for PIP-Net compatibility"
            )
    except Exception:
        # If imports fail or net is not ProtoQuantNet, continue normally
        pass
    ...
    with torch.no_grad():
        if args.state_dict_dir_net != "":
            ...
            if (
                ...
                logger.info(
                    "Classification layer initialized with mean %s",
...
def main_entry() -> None:
    ...
    logger.info("Starting purity benchmark; log_dir=%s", args.log_dir)
