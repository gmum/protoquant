...
def load_official_pipnet_model(
    num_classes: int, checkpoint_path: str
) -> OfficialPIPNet:
    # 1. Architecture Definition matching ConvNext_tiny_13
    args = Namespace(
        net="convnext_tiny_13", num_features=0, bias=False, disable_pretrained=True
    )
...
class OfficialPIPNetWrapper(nn.Module):
    def __init__(self, official_model: OfficialPIPNet):
        ...
        self.head = Namespace(
            classifier=self.model._classification, P=self.model._num_prototypes
        )
...
def run_official_benchmark(cfg: OfficialBenchConfig):
    ...
    print(
        f"Loading TRAIN dataset (for projection/purity) from: {cfg.cub_cropped_data_path}"
    )
