...
def eval_prototypes_cub_parts_csv(
    ...
    logger.info(
        f"Number of prototypes in parts_presences: {len(proto_parts_presences.keys())}"
    )
    ...
    for proto in proto_parts_presences.keys():
        ...
        if (
            ...
            logger.info(
                f"unused part in keys! {proto} {proto_parts_presences[proto].keys()} {proto_parts_presences[proto]}"
            )
            raise ValueError()
    ...
    logger.info(
        f"Mean purity of prototypes (corresponding to purest part): {np.mean(list(max_presence_purity.values()))} std: {np.std(list(max_presence_purity.values()))}"
    )
    ...
    metrics = {
        "purity_mean_purest_part": float(np.mean(list(max_presence_purity.values()))),
        "purity_std_purest_part": float(np.std(list(max_presence_purity.values()))),
        "purity_mean_most_present_part": float(
            np.mean(list(most_often_present_purity.values()))
        ),
        "purity_std_most_present_part": float(
            np.std(list(most_often_present_purity.values()))
        ),
        "purity_num_prototypes_in_csv": int(len(list(proto_parts_presences.keys()))),
        "purity_num_part_related_gt_0.5": int(prototypes_part_related),
    }
    return metrics
...
def get_topk_cub(net, projectloader, k, epoch, device, args):
    ...
    with open(csvfilepath, "w", newline="") as csvfile:
        logger.info("Writing CSV file with top k image patches..")
        writer = csv.writer(csvfile, delimiter=",")
        writer.writerow(
            [
                "prototype",
        ...
        for _, prototype in protoype_iter:
            ...
            if len(proto_img_coordinates) > 10000:
                ...
                proto_img_coordinates = []
        logger.info(
            f"Warning: image patches included in topk, but similarity < 0.1! This might unfairly reduce the purity metric because prototype has less than k similar image patches. You could consider reducing k for prototypes {too_small}"
        )
